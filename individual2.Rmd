...
```{r}
#defClean3 <- defClean %>%
 # select(Name, Opponent_Opponent, Statistic) %>%
  #spread(Opponent_Opponent, Tackles_Solo)
#defClean3 <- defClean3[, c("Kansas State", "West Virginia")]

#ggplot(data = defClean3, aes(x = `Kansas State`, y= `West Virginia`)) + geom_point() + ylab("Solo Tackles by player against Kansas State") + xlab("Solo Tackles by player against West Virginia") + ggtitle("Comparing ISU's defense against Kansas State and West Virginia")
```
**Part Two: Tidying (2)**
...
ggplot(data = New1, aes(x = Weight)) + geom_histogram( fill = "darkred", color = "black") + facet_wrap(~Position)
ggplot(data = New1, aes(x = Height_in_inches)) + geom_histogram( fill = "darkred", color = "black") + facet_wrap(~Position)
